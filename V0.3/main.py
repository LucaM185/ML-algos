...
for i in range(1000):
    ...
    # Train
    t, pred, loss = lr.train(mres, qres, 8, 0, 0.5, draw=1)  # n*m*q complexity draw = 0, 1, 2
    print(f"R2 = {round(lr.Rsquared(), 3)}, Loss: {loss[-1]}, {t}, m: {round(pred[0], 5)},"
          f" q: {round(pred[1], 5)} with parameters {a, b, c}")
